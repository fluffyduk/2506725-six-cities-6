...
import { AppExpectionFilter, ExceptionFilter, HttpErrorExceptionFilter, PathTransformer, ValidationExceptionFilter } from '../shared/libs/rest/index.ts';
...
export function createRestApplicationContainer() {
  const restApplicationContainer = new ContainerModule(({ bind }) => {
    ...
    bind<ExceptionFilter>(Component.ExceptionFilter).to(AppExpectionFilter).inSingletonScope();
    bind<ExceptionFilter>(Component.HttpExceptionFilter).to(HttpErrorExceptionFilter).inSingletonScope();
    bind<ExceptionFilter>(Component.ValidationExceptionFilter).to(ValidationExceptionFilter).inSingletonScope();
    bind<PathTransformer>(Component.PathTransformer).to(PathTransformer).inSingletonScope();
  });
